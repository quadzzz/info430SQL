--1) CONTROL OF FLOW
    ...
    1) Sequential structure - code is processed sequentially from the first line up to
    the last.
    3) conditional or selection structure - Only blocks of code that meet certian
    conditions are processed
    3) Repetition or Looping structure - the same block is executed repeatedly until
    certain codntion becomes false.
    CASE statements are similar to IF statements but are more preferrable if you have
    multiple alternatives
    IF...ELE IF -- ELSE IF.. ELSE
    Example: The RFM model is quite commonly used in marketing to measure customer
    value
...
7) LIMIT/TOP
*/
FROM (SELECT CustomerID, COUNT(*) AS numOrders, MAX(OrderDate) AS RecentOrderDate,
SUM(Calc_OrderTotal) AS Sales
FROM tblORDER
...
-- Stored procedures are database objects and are developed and precompiled for subsequent use.
/*
--Cornerstones of Object-Oriented Programming applicable to stored
procedures:
a). Modularity: Program code can be broken down into smaller modular
components
...
this is achieved by use of classes and methods in object-oriented
programming.
Likewise, when we define stored procedures with parameters and compile them,
we can use the stored procedure whenever we want just executing it and
passing
...
BEGIN
--define select query
SELECT c.Lname, c.Fname, o.CustomerID, SUM(o.Calc_OrderTotal) AS
OrderTotal
...
--Define Insert Query
INSERT INTO tblPRODUCT(ProductID, ProductName, ProdTypeID,
ProductDescr, Price)
